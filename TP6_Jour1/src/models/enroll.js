...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('courseForm').addEventListener('submit', async function(event) {
        ...
        const courses = {
            ...
            physics: {
                day: document.getElementById('physicsDay').value,
                time: document.getElementById('physicsTime').value,
            }
        };
        const schedule = {};
        let isValid = true;
        for (const [subject, { day, time }] of Object.entries(courses)) {
            ...
            if (schedule[key]) {
                isValid = false;
                alert(`Erreur: ${subject.charAt(0).toUpperCase() + subject.slice(1)} ne peut pas être pris le même jour et à la même heure qu'un autre cours.`);
                break;
            }
